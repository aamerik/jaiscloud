...
# Spark K8s cluster deploy-mode (EXECUTOR_MODE=k8s only)
JAISCLOUD_SPARK_K8S_CLUSTER_MODE=auto          # auto | always | never — when to engage cluster deploy-mode
JAISCLOUD_SPARK_K8S_STRIP_SCHEDULING=true      # strip scheduling fields from merged pod templates
JAISCLOUD_SPARK_K8S_CLUSTER_SHUTDOWN=leave     # leave | delete — what Close() does to cluster-mode Jobs
JAISCLOUD_SPARK_K8S_POD_TEMPLATE_MAX_BYTES=262144  # max size per pod-template YAML (256 KiB)
...
- **`buildJobManifest`** — returns `(batchJob, CloudSparkTransform, cleanupKey string, error)`. The transform is returned (not re-selected) so `Submit` can call `transform.DeleteTemplate` to clean up an uploaded executor pod template if `createJob` subsequently fails.
- **`jobEntry`** — tracks a submitted Job with `name`, `cleanupKey`, `transform`, and `isClusterMode bool`. `isClusterMode` is set at `Submit` time so `Close()` can apply different shutdown behaviour without any K8s API calls.
- **`CloudSparkTransform` / `CloudExecutorTemplateIO`** — per-cloud contributions to the K8s Job manifest. `CloudExecutorTemplateIO` sub-interface adds `UploadTemplate(ctx, blobs, key, data)`, `DeleteTemplate(ctx, blobs, key)`, and `DriverFetchEnv(key)` so each cloud can store executor pod templates in its own blob backend and inject the URI as a `--conf` entry.
