...
func TestMemoryMessageStore_SendReceiveDelete(t *testing.T) {
	...
	msgs, err := s.Receive(ctx, testQueue, 1, now)
	if err != nil {
	...
	msgs2, _ := s.Receive(ctx, testQueue, 1, now)
	if len(msgs2) != 0 {
		t.Fatalf("expected 0 (in-flight), got %d", len(msgs2))
	}

	// Delete
	if err := s.Delete(ctx, testQueue, msgs[0].ReceiptHandle); err != nil {
		t.Fatalf("Delete: %v", err)
	}

	// Queue should be empty
	msgs3, _ := s.Receive(ctx, testQueue, 1, now.Add(time.Minute))
	if len(msgs3) != 0 {
...
func TestMemoryMessageStore_VisibilityTimeout(t *testing.T) {
	...
	s.Send(ctx, newMsg("vis-test"))
	msgs, _ := s.Receive(ctx, testQueue, 1, now)
	if len(msgs) != 1 {
		t.Fatal("no messages received")
	}

	// Still in-flight 5s later
	msgs2, _ := s.Receive(ctx, testQueue, 1, now.Add(5*time.Second))
	if len(msgs2) != 0 {
		t.Fatal("message should still be in-flight")
	}

	// Visible again after timeout (30s default set by Receive)
	msgs3, _ := s.Receive(ctx, testQueue, 1, now.Add(31*time.Second))
	if len(msgs3) != 1 {
...
func TestMemoryMessageStore_ChangeVisibility(t *testing.T) {
	...
	if err := s.ChangeVisibility(ctx, testQueue, msgs[0].ReceiptHandle, 60, now); err != nil {
		t.Fatalf("ChangeVisibility: %v", err)
	}

	// Still invisible at 45s
	msgs2, _ := s.Receive(ctx, testQueue, 1, now.Add(45*time.Second))
	if len(msgs2) != 0 {
		t.Fatal("should still be invisible")
	}

	// Visible at 61s
	msgs3, _ := s.Receive(ctx, testQueue, 1, now.Add(61*time.Second))
	if len(msgs3) != 1 {
...
func TestMemoryMessageStore_ChangeVisibilityZero(t *testing.T) {
	...
	s.ChangeVisibility(ctx, testQueue, msgs[0].ReceiptHandle, 0, now)
	msgs2, _ := s.Receive(ctx, testQueue, 1, now)
	if len(msgs2) != 1 {
...
func TestMemoryMessageStore_Purge(t *testing.T) {
	...
	s.Send(ctx, newMsg("m1"))
	s.Send(ctx, newMsg("m2"))
	s.Purge(ctx, testQueue)

	msgs, _ := s.Receive(ctx, testQueue, 10, now)
	if len(msgs) != 0 {
...
func TestMemoryMessageStore_FIFODeduplication(t *testing.T) {
	...
	s.Send(ctx, m)
	s.Send(ctx, m) // duplicate — should be dropped

	msgs, _ := s.Receive(ctx, fifoQueue, 10, now)
	if len(msgs) != 1 {
...
func TestMemoryMessageStore_DelayedMessage(t *testing.T) {
	...
	// Visible after delay
	msgs2, _ := s.Receive(ctx, testQueue, 1, now.Add(11*time.Second))
	if len(msgs2) != 1 {
...
func TestMemoryMessageStore_ApproximateCounts(t *testing.T) {
	...
	s.Send(ctx, newMsg("m1"))
	s.Send(ctx, newMsg("m2"))
	s.Send(ctx, sqsstore.SQSMessage{
		MessageID:  "m3",
		QueueURL:   testQueue,
		Body:       "m3",
		SentAt:     now,
	...
	visible, notVisible, delayed, err := s.GetApproximateCounts(ctx, testQueue, now)
	if err != nil {
