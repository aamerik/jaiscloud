...
type DynamoDBItemStore interface {
	// ── Data-plane methods ───────────────────────────────────────────────────
	PutItem(ctx context.Context, account, region, table, pkHash string, item map[string]any, cond ConditionSpec) (map[string]any, error)
	GetItem(ctx context.Context, account, region, table, pkHash string) (map[string]any, error)
	DeleteItem(ctx context.Context, account, region, table, pkHash string, cond ConditionSpec) (map[string]any, error)
	UpdateItem(ctx context.Context, account, region, table, pkHash string, item map[string]any, spec UpdateSpec) (map[string]any, error)
	...
	Query(ctx context.Context, account, region, table string, q QuerySpec) ([]map[string]any, int, string, error)
	...
	Scan(ctx context.Context, account, region, table string, s ScanSpec) ([]map[string]any, int, string, error)
	BatchWriteItems(ctx context.Context, account, region string, reqs []BatchWriteRequest) ([]BatchWriteRequest, error)
	BatchGetItems(ctx context.Context, account, region string, reqs []BatchGetRequest) (map[string][]map[string]any, error)
	...
	CreateTableSchema(ctx context.Context, account, region string, schema TableSchema) error

	// DropTableSchema removes per-table storage and all its items/index rows.
	// Called by TableProvider.DeleteTable.
	DropTableSchema(ctx context.Context, account, region, tableName string) error

	// AddGSI backfills index rows for a new GSI from existing items.
	// Called by TableProvider.UpdateTable when creating a GSI.
	AddGSI(ctx context.Context, account, region, tableName string, schema TableSchema, idx IndexDef) error

	// DeleteGSI removes all index rows for the named GSI.
	// Called by TableProvider.UpdateTable when deleting a GSI.
	DeleteGSI(ctx context.Context, account, region, tableName string, schema TableSchema, indexName string) error
}
