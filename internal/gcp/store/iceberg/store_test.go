...
func runStoreTests(t *testing.T, s Store) {
	...
	props, err = s.UpdateNamespaceProperties(ctx, "db", []string{"owner"}, map[string]string{"env": "prod"})
	if err != nil || props["owner"] != "" || props["env"] != "prod" {
		t.Fatalf("update properties: %v %v", err, props)
	}
