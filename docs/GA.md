...
**25 services, 547 operation/transport cells.** The canonical sources are
...
| --- | ---: | ---: | ---: | ---: |
| REST (JSON, Discovery-backed) | 243 | 87 | 37 | 12 |
| gRPC (proto descriptors + official-client conformance) | 160 | 8 | 0 | 0 |
...
- **gRPC** — official `cloud.google.com/go` clients: **165/165 checks pass** (Datastore 11,
  Firestore 18, IAM 3, KMS 31, Logging 5, Monitoring 24, Operations 5, Pub/Sub 25,
  Secret Manager 16, Storage 23, Workflow Executions 4) against `:8081`.
...
- **gRPC** — **8** of **168** cells remain `limited`: verified against proto descriptors only.
  The other **160** are `ga`, verified with the official `cloud.google.com/go` clients against a
  live emulator (Datastore 8, Storage 23, IAM 3, KMS 31, Secret Manager 15, Logging 5,
  Operations 5, Pub/Sub 25, Firestore 17, Monitoring 24, Workflow Executions 4). The conformance
  harness exercises 165 checks over those 160 proto methods (Datastore 11, Firestore 18, IAM 3,
  KMS 31, Monitoring 24, Operations 5, Storage 23, Secret Manager 16, Workflow Executions 4, the
  rest one per method). KMS
