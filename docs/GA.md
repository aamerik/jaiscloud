...
**25 services, 683 operation/transport cells.** The canonical sources are
...
| --- | ---: | ---: | ---: | ---: |
| REST (JSON, Discovery-backed) | 247 | 94 | 37 | 9 |
| gRPC (proto descriptors + official-client conformance) | 240 | 12 | 0 | 44 |
...
- **gRPC** — official `cloud.google.com/go` clients: **257/257 checks pass** (Dataproc 14,
  Datastore 11, Eventarc 12, Firestore 18, Functions 18, IAM 6, KMS 31, Logging 5, Managed Kafka 18,
  Metastore 13, Monitoring 24, Operations 5, Pub/Sub 25, Resource Manager 4, Secret Manager 16,
  Service Usage 5, Storage 23, Workflow Executions 4, Workflows 5) against `:8081`. The IAM probes
...
- **gRPC** — **12** of **257** cells remain `limited`: verified against proto descriptors only.
  The other **228** are `ga` (plus **17** explicit `unsupported` stubs: Dataproc `DiagnoseCluster`,
  Functions `CallFunction`/`ListRuntimes`, Service Usage `BatchGetServices`, Workflows
  `ListWorkflowRevisions`, Cloud Resource Manager's 7 project lifecycle/lookup RPCs, and
  Dataproc Metastore's 5 deferred control-plane RPCs — `ExportMetadata`, `RestoreService`,
  `QueryMetadata`, `MoveTableToDatabase`, `AlterMetadataResourceLocation`),
  ...
  Managed Kafka 17, Metastore 13, Operations 5, Pub/Sub 25, Resource Manager 4, Firestore 17,
  Monitoring 24, Workflow Executions 4, Workflows 5, Functions 10, Service Usage 5). The conformance
  harness exercises 242 checks over those 228 proto methods (Dataproc 14, Datastore 11, Firestore 18,
  Functions 18, IAM 3, KMS 31, Managed Kafka 18, Metastore 13, Monitoring 24, Operations 5, Pub/Sub 25,
  Resource Manager 4, Storage 23,
...
- **Explicit `Unimplemented` stubs (graded `unsupported` in the matrix)** — Dataproc Metastore
  ...
  provider CRUD is the whole implemented surface).
  These fail loud
