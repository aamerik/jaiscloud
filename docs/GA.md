...
**23 services, 483 operation/transport cells.** The canonical sources are
...
| --- | ---: | ---: | ---: | ---: |
| REST (JSON, Discovery-backed) | 198 | 81 | 37 | 3 |
| gRPC (proto descriptors) | 0 | 164 | 0 | 0 |
...
The entire gRPC surface is `limited` today, pending full proto-conformance (see §7).
...
- **gRPC** — official `cloud.google.com/go` clients: **23/23 checks pass** (Firestore 3, KMS 6,
  Pub/Sub 4, Secret Manager 6, Storage 4) against `:8081`.
- **`gcloud` CLI** — **47 commands: 46 pass, 0 fail, 1 unsupported, 0 regressions**
  (gcloud 585.0.0). The one unsupported command is `gcloud functions list`, because gcloud 585
  targets the Cloud Functions **v2** API while the emulator serves v1 (see §7).
- **Conformance harnesses** — REST wire-conformance (**11 `info` findings, all `unknown_field`,
  zero high-severity**), gRPC message-level, gcloud CLI, and the fidelity-matrix drift check.

The 11 REST findings are all `info`: BigQuery list responses (8), a Cloud DNS project quota
field (1), and KMS key-ring IAM metadata (2) the emulator emits beyond the Discovery schema's
modelled set. The gate fails only on high-severity divergence, so these are recorded, not fatal.
...
- **Cloud Functions** — **v1 surface only**. The single `gcloud` gap is `gcloud functions list`,
  which targets the Cloud Functions v2 API and is recorded `unsupported`. `Function.GetLocation`
  is `limited` (no matching Discovery method).
- **gRPC** — all **164** cells are `limited`: verified against proto descriptors only, pending
  full proto-conformance.
...
contrast, records the transcript against a live emulator first — see §4.)
