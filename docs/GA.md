...
**25 services, 644 operation/transport cells.** The canonical sources are
...
| gRPC (proto descriptors + official-client conformance) | 228 | 12 | 0 | 17 |
...
- **gRPC** — official `cloud.google.com/go` clients: **242/242 checks pass** (Dataproc 14,
  ...
  Service Usage 5, Storage 23, Workflow Executions 4, Workflows 5) against `:8081`.
...
- **Approximated, not modelled** — Cloud Workflows executes synchronously, ignores
  ...
  Eventarc has no event-delivery engine; Managed Kafka `UpdateCluster`/`UpdateTopic`/`UpdateAcl`
