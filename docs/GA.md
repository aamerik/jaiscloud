...
**25 services, 602 operation/transport cells.** The canonical sources are
...
| gRPC (proto descriptors + official-client conformance) | 201 | 12 | 0 | 3 |
...
- **gRPC** — official `cloud.google.com/go` clients: **215/215 checks pass** (Dataproc 14,
  Datastore 11, Firestore 18, Functions 18, IAM 3, KMS 31, Logging 5, Managed Kafka 18,
  Monitoring 24, Operations 5, Pub/Sub 25, Secret Manager 16, Storage 23,
...
- **gRPC** — **12** of **216** cells remain `limited`: verified against proto descriptors only.
  The other **201** are `ga` (plus **3** explicit `unsupported` stubs, Dataproc `DiagnoseCluster`
  and Functions `CallFunction`/`ListRuntimes`),
  ...
  Workflow Executions 4, Functions 11). The conformance
  harness exercises 215 checks over those 201 proto methods (Dataproc 14, Datastore 11, Firestore 18,
  Functions 18, IAM 3, KMS 31, Managed Kafka 18, Monitoring 24, Operations 5, Storage 23,
  Secret Manager 16, Workflow Executions 4, the rest one per method). KMS
