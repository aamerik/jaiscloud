...
**25 services, 590 operation/transport cells.** The canonical sources are
...
| gRPC (proto descriptors + official-client conformance) | 191 | 12 | 0 | 1 |
...
- **gRPC** — official `cloud.google.com/go` clients: **197/197 checks pass** (Dataproc 14,
  Datastore 11, Firestore 18, IAM 3, KMS 31, Logging 5, Managed Kafka 18, Monitoring 24,
  Operations 5, Pub/Sub 25, Secret Manager 16, Storage 23, Workflow Executions 4) against `:8081`.
...
- **gRPC** — **12** of **204** cells remain `limited`: verified against proto descriptors only.
  The other **191** are `ga` (plus **1** explicit `unsupported` stub, Dataproc `DiagnoseCluster`),
  ...
  Workflow Executions 4). The conformance
  harness exercises 197 checks over those 191 proto methods (Dataproc 14, Datastore 11, Firestore 18,
  IAM 3, KMS 31, Managed Kafka 18, Monitoring 24, Operations 5, Storage 23, Secret Manager 16,
  Workflow Executions 4, the rest one per method). KMS
...
- **Explicit `Unimplemented` stubs (graded `unsupported` in the matrix)** — Dataproc Metastore
  ...
  Dataproc `DiagnoseCluster`. These fail loud
...
- **Approximated, not modelled** — Cloud Workflows executes synchronously, ignores
  ...
  merge the supplied `update_mask` fields but ignore the mask itself.
